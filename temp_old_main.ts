...
export class MinimapView extends ItemView {
    ...
    async onOpen() {
        ...
        if (activeView) {
            this.editor = activeView.editor.cm;


			// Add the debug log here
			console.log("Fetched editor instance:", this.editor);
			console.log("Type of activeView.editor:", typeof activeView.editor);
			...
			// Set the font size for the minimap
			this.setMinimapFontSize();
    ...
    renderMinimap() {
		...
		// Add the debug log here
		console.log("Rendering minimap with content:", content);
	...
	setMinimapFontSize() {
		...
		const ratio = minimapHeight / editorHeight;
	
		// Assuming the default font size is 16px, adjust based on the ratio.
		const fontSize = 16 * ratio;
	
		this.minimapContainer.style.fontSize = `${fontSize}px`;
	}
...
// creating a new class for the Minimap
